...
class GCPStorageProvider(CloudStorageProvider):
    """☁️ GCP-specific cloud storage provider"""
    def __init__(self):
        self.bucket_name = os.getenv('GCP_BUCKET_NAME')

    def upload_file(self, file_path: str) -> str:
        # 🔄 Import the upload function (if needed, change 'gcp_toolkit' to 'ciberfobia-api')
        from services.gcp_toolkit import upload_to_gcs
        return upload_to_gcs(file_path, self.bucket_name)

class S3CompatibleProvider(CloudStorageProvider):
    """☁️ S3-compatible storage provider (e.g., DigitalOcean Spaces)"""
    def __init__(self):
    ...
    def upload_file(self, file_path: str) -> str:
        # 🔄 Import the upload function (if needed, change 's3_toolkit' to 'ciberfobia-api')
